...
export default function SuperAdminDashboard({ auth, stats, recentOrders, topTenants }) {
  ...
  const safeTopTenants   = Array.isArray(topTenants)   ? topTenants   : [];
                ...
                <div className="space-y-4">
                  {safeRecentOrders.length ? (
                    safeRecentOrders.map((order) => {
                      ...
                      return (
                        <div key={order?.id ?? crypto.randomUUID()} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
