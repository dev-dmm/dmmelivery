...
// Main Component
export default function SettingsIndex({ auth, tenant, courier_options, business_types, currency_options }) {
  ...
  const { canMakeCall, recordCall } = useRateLimit(20, 60000); // 20 calls per minute

  // Defensive props with proper defaults
  ...
  // Message management with auto-clear
  ...
  const copyToClipboard = useCallback(async (text, section = 'general') => {
    try {
      if (!text) {
  ...
  const handleBusinessUpdate = useCallback(async () => {
    ...
    try {
      ...
      const result = await apiService.post(route('settings.business.update'), sanitizedData);
  ...
  const handleACSCredentialsUpdate = useCallback(async () => {
    ...
    try {
      ...
      const result = await apiService.post(route('settings.courier.acs.update'), sanitizedData);
  ...
  const testCourierConnection = useCallback(async (courier) => {
    if (!canMakeCall()) {
  ...
  // WooCommerce bridge functionality
  ...
  const testWooBridge = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok && data?.success) {
        const message = `Bridge test successful: order_id ${data.order_id}${data.shipment_id ? `, shipment_id ${data.shipment_id}` : ''}`;
  ...
  // Courier status badge component
  ...
  const getMessageAlert = useCallback((section) => {
  ...
  // Clean up message timers on unmount
  useEffect(() => {
    return () => {
      // Clear any pending message timeouts when component unmounts
      setMessages({});
    };
  }, []);
                      ...
                      <div className="border-t pt-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div>
                            ...
                            <select
                              id="default_currency"
                              {...businessForm.register('default_currency')}
                              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500"
                            >
                          ...
                          <div>
                            ...
                            <TextInput
                              ...
                              className="mt-1 block w-full"
                              aria-invalid={!!businessForm.formState.errors.tax_rate}
                            />
                            {businessForm.formState.errors.tax_rate && (
                              <p className="mt-1 text-sm text-red-600">
                                {businessForm.formState.errors.tax_rate.message}
                              </p>
                            )}
                          </div>
                          ...
                          <div>
                            ...
                            <TextInput
                              ...
                              step="0.01"
                              min="0"
                              {...businessForm.register('shipping_cost', { valueAsNumber: true })}
                              className="mt-1 block w-full"
                              aria-invalid={!!businessForm.formState.errors.shipping_cost}
                            />
                            {businessForm.formState.errors.shipping_cost && (
                              <p className="mt-1 text-sm text-red-600">
                                {businessForm.formState.errors.shipping_cost.message}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="mt-4 space-y-3">
                          <label className="flex items-center">
                            <input
                              type="checkbox"
                              {...businessForm.register('auto_create_shipments')}
                          ...
                          <label className="flex items-center">
                            <input
                              type="checkbox"
                              {...businessForm.register('send_notifications')}
                              className="rounded border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500"
                            />
                          ...
                          <div key={key} className="border rounded-lg p-6">
                            <div className="flex items-center justify-between mb-4">
                              <div className="flex items-center space-x-3">
                                <span className="text-2xl" aria-hidden="true">{courier.logo}</span>
                                <div>
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-sm text-gray-500">
                                {apiToken ? 'Token is configured' : 'No token generated'}
                              </p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          <div className="bg-white border rounded-lg p-4">
                            ...
                            <div className="flex items-center gap-2 mb-4">
                              ...
                              <SecondaryButton 
                                onClick={() => copyToClipboard(wooEndpoint, 'woo')}
                                aria-label="Copy WooCommerce endpoint to clipboard"
                              >
                                <ClipboardDocumentIcon className="-ml-1 mr-2 h-4 w-4" />
                                Copy
                              </SecondaryButton>
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="space-y-2">
                                <div className="flex items-center gap-2">
                                  <code className="text-xs bg-gray-50 px-2 py-1 rounded break-all flex-1">
                                    X-Api-Key: {maskedToken}
                                  </code>
                                  {apiToken && (
                                    <SecondaryButton 
                                      onClick={() => copyToClipboard(apiToken, 'woo')}
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  {tenantId && (
                                    <SecondaryButton 
                                      onClick={() => copyToClipboard(tenantId, 'woo')}
                              ...
                              <p className="text-xs text-gray-500 mt-2">
                                The WooCommerce plugin should send orders to this endpoint using these headers. 
                          ...
                          <div className="bg-white border rounded-lg p-4">
                            ...
                            <PrimaryButton 
                              onClick={testWooBridge} 
                              disabled={loading.woo_test || !apiToken || !tenantId}
                            >
                              {loading.woo_test ? (
                                <>
                                  <ArrowPathIcon className="animate-spin -ml-1 mr-2 h-4 w-4" />
