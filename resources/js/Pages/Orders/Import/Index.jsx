...
export default function ImportIndex({ auth, recentImports, stats, supportedFormats, maxFileSize }) {
  ...
  const pollActiveImports = async (importIds) => {
    try {
      const responses = await Promise.all(
        importIds.map(id =>
          fetch(route('orders.import.status', { importId: id }))
            .then(res => res.json())
        )
      );
  ...
  const handleUpload = () => {
    ...
    xhr.onload = () => {
      ...
      let response;
      try { response = JSON.parse(xhr.responseText); } catch { response = { success: false, message: 'Invalid response' }; }
  ...
  const getProgressBar = (import_) => {
    if (!['processing', 'completed', 'partial'].includes(import_.status)) return null;
                      ...
                      {['completed', 'partial', 'failed'].includes(displayImport.status) && (
                        <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          <div>
                            ...
                            <span className="ml-1 font-medium text-green-600">{import_.orders_created}</span>
                          </div>
                          <div>
                            <span className="text-gray-500">Orders Updated:</span>
                            <span className="ml-1 font-medium text-blue-600">{import_.orders_updated}</span>
                          </div>
                          <div>
                            <span className="text-gray-500">Success Rate:</span>
                            <span className="ml-1 font-medium">{import_.success_rate}%</span>
                          </div>
                          {import_.has_errors && (
                            <div>
